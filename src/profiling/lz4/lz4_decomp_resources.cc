...
{
    /* Free tasks */
    for (doca_compress_task_decompress_lz4_block* task : tasks)
        doca_task_free(doca_compress_task_decompress_lz4_block_as_task(task));
...
lz4_decomp_resources::prepare_memory(const lz4_decomp_config& cfg)
{
    ...
    size_t input_buf_size = cfg.input_size;
    size_t output_buf_size = MAX_BUF_SIZE;
    ...
    memcpy(mmap_buffer, cfg.data, input_buf_size);
...
doca_error_t
lz4_decomp_resources::setup_comp_ctx(
...
{
    doca_error_t status;
    status = doca_compress_create(dev, &comp);
    if (status != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to create comp: %s", doca_error_get_descr(status));
        return status;
    }

    status = doca_compress_task_decompress_lz4_block_set_conf(
      comp, success_cb, error_cb, MAX_NB_AG_TASKS);
    if (status != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to set lz4 decomp task conf: %s",
                     doca_error_get_descr(status));
        return status;
    }

    ctx = doca_compress_as_ctx(comp);
    if (!ctx) {
        DOCA_LOG_ERR("Failed to convert comp to ctx");

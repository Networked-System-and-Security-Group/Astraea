...
DOCA_LOG_REGISTER(LOCALEC:HOST : MAIN);
...
int main(int argc, char **argv) {
    ...
    /* Default: 320MB = 160MB src half + 160MB dst half.
     * 160MB covers max-size request (128MB data + 32MB rdnc). */
    HostCfg cfg = {
        .ibdevName = "mlx5_0",
        .dpuIp = "192.168.100.2",
        .port = 22545,
    ...
    CHECK_RETURN(doca_argp_init("localec_host", &cfg), "init argp");
    ...
    memset(memAddr, 0xCD, memSizeBytes);
    ...
    DOCA_LOG_INFO("Memory exported. DPU may now run localec. Press Ctrl-C to exit.");
    while (gRunning) {
        pause();
    }
